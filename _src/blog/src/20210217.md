...
announce it and compare notes on Rust actor systems in general.
...
However the fact that there are a proliferation of actor crates shows
...
streamlined for in the code.  The actor model on the other hand is the
opposite.  In the purest actor model, nothing blocks.  By default
everything is asynchronous and events can arrive from any source at
any time.  Multiple calls can be outstanding on an actor, not just one
as in the case of `await`.

For things that are asynchronous in a big way with events arriving
from all directions, or where trying to force things down a mostly
sequential code path is just not going to work out, or where it is
...
My own crate [**Stakker**] takes a pure "nothing blocks" approach.
Once an actor is up and running, nothing blocks &mdash; ever.  Events
arriving at an actor must be handled immediately.  If handling an
event starts a process that may take some time, then the actor must
arrange to be notified when that process is complete.  This is very
clean and easy to reason about.

However it seems that other actor implementations make various
compromises to facilitate interop with async/await.  For example an
actor may block the processing of its own queue whilst it waits for an
external process to complete, e.g. an `await` call on something from
the async/await ecosystem.  This is a slightly less pure
interpretation of the actor model and does have a few downsides,
risking delays of unrelated processes and maybe even deadlocks if the
coder isn't careful.  It also complicates the queuing.

If we have an awaitable object and wish to interface it to an actor
system, the cleanest way is to wrap it in an actor and have that actor
accept calls and queue them up internally, and feed them one by one to
the awaitable object, i.e. sequentially.  This is because you can't
have two awaits running at the same time on an awaitable object, which
is a limitation that actors don't have.


## When multi-threaded is slower than single-threaded

It's well known with **rayon** for example, that if your unit of work
is too small, then parallelizing the job will make it slower.  So even
...
single-threaded at its core (although several independent [**Stakker**]
runtimes can be run on different threads).  This does shift the burden
of dividing work between threads back to the coder, a job which the
coder would perhaps prefer not to have to think about.  However if you
care about efficiency, then you *have* to think about it.  And in any
case, without a lot of care, throwing more threads at an actor system
will just make it slower.  So typical actor code will likely run
faster single-threaded anyway.
...
coding, despite all the complications it brings.  But this has been
reduced to "single-threaded bad, multi-threaded good", so people see a
multi-threaded solution and think that it must be better.  But that's
just not true.  The whole thing is a lot more nuanced than that.
Really it comes down to: How big is your unit of work? i.e. how much
work can you get done between synchronization points.
...
So given that for an actor work-load, the unit of work is typically
small compared to synchronization overheads, running multiple
single-threaded actor runtimes is the way to go.  Load-balancing is
...
Here are some of the features that make [**Stakker**] different to some
other Rust actor systems:
...
- **Event system independent:** It is not tied to any particular
  ...
  Already implemented is `mio`, but it would be possible to layer it
...
- A Lua actor system with actor coroutines was developed over 2012-2018
...
- This was redesigned and rewritten as the open-source [**Stakker**]
  crate later that year, with first release in Jan-2020
